...
class DataSet:
    ...
    AA = ["B-ORG","B-MISC","B-PER","I-PER","B-LOC","I-ORG","I-LOC","I-MISC"]
    AL_TAGS = ["O","B-ORG","B-MISC","B-PER","I-PER","B-LOC","I-ORG","I-LOC","I-MISC"]
    RH_TAGS = ["O","ORG","MISC","PER","LOC"]
    ...
    def loadData(self,fpath):
        ...
        for i in fi.readlines():
            a = i.strip("\n").split()
            if len(a) <=1 and len(words_tmp) != 0:
            ...
            if len(a)!=4:
                continue
            words_tmp.append(a[0])
            tags_tmp.append(a[3])
            if a[3] in DataSet.AA:
                rtags_tmp.append(a[3][2:]) 
            else:
                rtags_tmp.append(a[3])
    ...
    def getdata(self,use_size,batch_size,single_word_len):
        ...
        for i in range(batch_size):
            while len(ws[i])<max_len:
                ...
                ts[i].append(self.tagToId["O"])
                rts[i].append(self.rtagToId["O"])
        ...
        for i in range(len(ws)):
            ...
            for j in ws[i]:
                ...
                if m>single_word_len:
                    ...
                    for kk in range(m):
                        k[(single_word_len-m)/2+kk]=self.CharToId[self.IdToWord[j][kk].lower()]
                ar.append(k)
            c_ws.append(ar)
    ...
    def normData(self,words,tags,rtags,single_word_len):
        ...
        for i in range(len(words)):
            while len(words[i])<max_len:
                ...
                tags[i].append(self.tagToId["O"])
                rtags[i].append(self.rtagToId["O"])
